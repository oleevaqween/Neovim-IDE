...
Neovim is only half of this setup. tmux is what turns a single terminal window into a full
workspace, and this config leans on it hard.

<p align="center">
  <img src="assets/tmux-session.png" width="90%" alt="A tmux session with nvim, shell, and minikube windows" />
</p>

<p align="center">
  <em>One tmux session ("Devops"), four windows: a shell, this Neovim IDE editing a multi-service
  project, vimtutor, and a minikube pane, all switchable in a keystroke.</em>
</p>

That screenshot is a real session, not a staged demo. Notice what's actually going on:

- **One session per project, many windows inside it.** `Devops` groups everything for that
  project: `1:bash`, `2:ide` (Neovim), `3:vimtutor`, `4:minikube`. No more juggling a pile of
  separate terminal tabs or losing track of which shell was running what.
- **The status bar tells you where you are without asking.** Session name on the left; on the
  right, the current git branch (colored red when dirty, green when clean), the date/time, the
  hostname, and a shortened working directory, all generated by the custom scripts in
  `tmux/scripts/`.
- **Panes inside `1:bash` show `(2)`** meaning that window itself is split into two panes, so a
  single window can hold a running process and a free shell side by side.
- **Neovim and tmux share one set of navigation keys.** Thanks to vim-tmux-navigator,
  `<C-h/j/k/l>` moves between Neovim splits *and* tmux panes interchangeably, so switching from
  editing `routes.js` to the shell to check `minikube status` doesn't require thinking about
  which multiplexer you're currently inside.
- **None of it disappears on reboot.** tmux-resurrect and tmux-continuum checkpoint the whole
  layout, including which directories each pane was in and the Neovim session running inside it,
  and restore it automatically the next time tmux starts.

This is the difference between "an editor with a terminal next to it" and a real terminal-based
IDE: the editor, the shell, and whatever else you're running (here, a Kubernetes cluster via
minikube) all live in the same multiplexed, persistent, git-aware workspace.
